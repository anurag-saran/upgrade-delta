...
| `10-reports-pvc.yaml` | PVC | Shared `upgrade-delta-reports` volume — the pipeline's workspace **and** the viewer's docroot. Must be **ReadWriteMany**. |
...
Get the scorecard URL from **Networking → Routes → `scorecard`**. It's empty until the
first PipelineRun writes reports; after a run, open:
