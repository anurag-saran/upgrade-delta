...
print()
print(f"   {B}closing obligations at deployment stage:{R}")
print(f"     canary: routing 5% of traffic to one instance ...", flush=True)
time.sleep(0.6)
print(f"     canary: error rate nominal, p99 within band over observation window")
print(f"     {GN}canary: CLOSED{R}")
print(f"     rollback-path: previous artifact present, procedure dry-run ok")
